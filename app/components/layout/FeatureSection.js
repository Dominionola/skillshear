      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f) => (
            <div
              key={f.title}
              className="bg-blue-600 rounded-2xl p-6 shadow-sm transition-shadow hover:shadow-md hover:bg-[radial-gradient(circle,hsla(224,89%,83%,1)_0%,hsla(0,0%,100%,1)_100%)]"
            >
              ...
              <p className="text-lg text-white">{f.desc}</p>
            </div>
          ))}
        </div>
      </div>
